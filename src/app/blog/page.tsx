...
export const metadata: Metadata = {
  title: 'Blog',
  description: 'Ankara nakliye, taşımacılık ve eşya taşıma hakkında faydalı bilgiler.',
  alternates: { canonical: 'https://ankarparcaesyanakliye.com.tr/blog' },
}
    ...
    <SiteLayout>
      <div className="bg-primary-600 py-12">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-white/70 mt-2">Nakliye ve taşımacılık hakkında faydalı bilgiler</p>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 py-12">
        {posts && posts.length > 0 ? (
              ...
              <Link key={post.id} href={`/blog/${post.slug}`} className="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all border border-gray-100">
                ...
                <div className="p-5">
                  <div className="flex items-center gap-1 text-gray-400 text-xs mb-2">
                    <Calendar size={12} />
                    {post.published_at ? new Date(post.published_at).toLocaleDateString('tr-TR') : new Date(post.created_at).toLocaleDateString('tr-TR')}
                  </div>
          ...
          <p className="text-center text-gray-400 py-12">Henüz blog yazısı eklenmemiş.</p>
        )}
      </div>
    </SiteLayout>
  )
